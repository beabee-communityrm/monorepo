...
import { cardInfo, oneTimeSignUp } from "@beabee/test-utils/test-data";
...
test("Join Flow", async ({ page, browserName }) => {
  ...
  await test.step("One-time payment", async () => {
    ...
    ).toBeEnabled(); // If the form was filled correctly, button should be enabled
    ...
    await page.waitForRequest(
      (request) =>
        request.method() === "POST" &&
        request.url().includes("/signup/complete"),
      { timeout: 30000 },
    );
  ...
  await test.step("Check contribution", async () => {
