...
export const listApiKeys = async (): Promise<void> => {
  await runApp(async () => {
    ...
    for (const key of apiKeys) {
      ...
      console.log(`Created by: ${key.creator ? `${key.creator.firstname} ${key.creator.lastname} (${key.creator.email})` : 'Unknown'}`);
      console.log(`Created at: ${key.createdAt.toISOString()}`);
      console.log(`Expires: ${key.expires ? key.expires.toISOString() : 'Never'}`);
      console.log('--------------------------------------------------');
    }
  });
}; 
