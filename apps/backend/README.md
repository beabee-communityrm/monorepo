...
```bash
yarn typeorm-cli migration:generate /opt/packages/core/src/migrations/<MigrationName> # The migration file is generated inside the docker container
...
yarn typeorm-cli migration:run # Runs the migration
```

For manual migrations (e.g. data migrations that TypeORM can't generate), create an empty migration file:

```bash
yarn typeorm-cli migration:create /opt/packages/core/src/migrations/<MigrationName>
```
