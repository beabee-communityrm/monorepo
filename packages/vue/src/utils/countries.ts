 ...
 *
 * @param locale Locale to resolve and sort the country names in
 */
export function getCountryItems(locale: string): SelectItem<string>[] {
  const tag = locale.split('@')[0]; // needed for non-standard locale names, like de@easy and de@informal
  const names = new Intl.DisplayNames([tag], { type: 'region' });
  return COUNTRY_CODES.map((id) => ({ id, label: names.of(id) || id })).sort(
    (a, b) => a.label.localeCompare(b.label, tag)
  );
}
